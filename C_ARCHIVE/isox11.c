...
void x11winope_(int WIDTH, int HEIGHT,int *maxcols,int *isocols, int *rc,
   ...
   *rc = -2;
   ...
   (void) fprintf(isolog,"ISOX11.C: x11winope about to connect to display %s\n",isodisplayname);
   (void)  printf(       "ISOX11.C: x11winope about to connect to display %s\n",isodisplayname);
   ...
   fprintf(isolog,"ISOX11.C: x11winope: Failure in XopenDisplay:\n Width, Height,maxcols, isocols, result=%8d%8d%8d%8d%8d\n",WIDTH,HEIGHT,*maxcols,*isocols,*rc);
    printf(       "ISOX11.C: x11winope: Failure in XopenDisplay:\n Width, Height,maxcols, isocols, result=%8d%8d%8d%8d%8d\n",WIDTH,HEIGHT,*maxcols,*isocols,*rc);
       ...
       *rc=0;
   fprintf(isolog,"ISOX11.C: x11winope has a good window\n");
    printf(       "ISOX11.C: x11winope has a good window\n");
   mapping=0;
   if(WIDTH < 0)
               {
                mapping=-1;
...
#include "keydefs.h"
/*
                 printf_("Key %d pressed in x11mouse..!\n",*nbut);
*/
                 *mousex=0;
                 *mousey=0;
                 *newx=0;
