...
c
      integer(4),allocatable,dimension(:) :: ix,iy
c
      integer(4) nval(0:255)
c
...
c     write(*,*)txtdata
      ...
      allocate(ix(ixdim*iydim))
      allocate(iy(ixdim*iydim))
c
      picdata=char(0)
      ix=0
      iy=0
c
     ...
     *       ,access='stream',status='old')
      ...
      do i=16,271
         picdata(i,0)=char(i-16)
      enddo
      write(40)picdata
      write(*,118)((mx,my,ichar(picdata(mx,my)),mx=0,63)
     *                                         ,my=0,0)
      stop
  6   continue
c
...
c
     ...
     *               ,clearline,fblack)
         write(20,114)nx,nvalind,nvalmax,average
     *              ,fblack,clearline,(nval(mx),mx=0,30)
         if(clearline)picdata(nx,:)=char(0)
      enddo
c
      do nx=ixdim-1,ixdim-maxcrop-1,-1
         nval=0
         do ny=0,iydim-1
            nval(ichar(picdata(nx,ny)))=nval(ichar(picdata(nx,ny)))+1
         enddo
...
c
     ...
     *               ,clearline,fblack)
         write(20,116)ny,nvalind,nvalmax,average
     *              ,fblack,clearline,(nval(mx),mx=0,30)
         if(clearline)picdata(:,ny)=char(0)
      enddo
c
      do ny=iydim-1,iydim-maxcrop-1,-1
         nval=0
         do nx=0,ixdim-1
            nval(ichar(picdata(nx,ny)))=nval(ichar(picdata(nx,ny)))+1
         enddo
