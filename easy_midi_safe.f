...
c
     ...
     *  )then
             i=i+1
             call easy_midi_numerics(i,i,maxrep)
...
c
         track_data(mt(idt)+nx  ,idt)=char(z'00')
         track_data(mt(idt)+nx+1,idt)=char(z'FF')
         track_data(mt(idt)+nx+2,idt)=char(z'2F')	! End track event
         track_data(mt(idt)+nx+3,idt)=char(z'00')
...
120   format('Stack reset from',i3,' to',i3)
