...
c
      ...
      do iy=0,iydim-1
         do ix=0,ixdim-1
            ...
            cogx=cogx+float(level*ix)
            cogy=cogy+float(level*iy)
            if(level.gt.levelmax)levelmax=level
            if(level.lt.levelmin)levelmin=level
            nval(level)=nval(level)+1
            moment=moment+level
            if(level.gt.0)nonzero_pels=nonzero_pels+1
         enddo
      enddo
      ncount0=(ixdim*iydim)-nonzero_pels
      rlevel=1./real(moment)
      icogx=0.5+(cogx*rlevel)
      icogy=0.5+(cogy*rlevel)
c
      npels=0
      mval=0
      nz=nonzero_pels/2
      do n=255,0,-1
         npels=npels+nval(n)
         mval(n)=npels ! Cumulative distribution function 
...
c
     ...
     *            ,maxnzp,maxbar,balance,median,icogx,icogy
