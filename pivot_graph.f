...
c
      write(*,109)
      read(*,*)npsi_degrees   !   ,nside
      xmin=-ndegrees+npsi_degrees
      xmax= ndegrees+npsi_degrees
      npstanza=0
      ymin=0.5
      ymax=1.2
      ...
      psi=float(npsi_degrees)*deg2rad
      px1= 1.0
      py1=-3.0
      angle_p =-atan(px1/py1)   ! Between Y-axis and OP
      radius_p=sqrt(px1**2+py1**2)  ! Distance from origin to P
c     px=px1
c     py=py1
      px= radius_p*sin(angle_p+psi)
      py=-radius_p*cos(angle_p+psi)
c
      write(*,110)radius_p,angle_p*rad2deg,float(npsi_degrees),px,py
c
      do nx=0,5
         nfillcol=nfillcol+1
         sx1=-.7+(0.05*float(nx))
c        sx1=sx1*float(nside)
         nlinecol=-1
         do ny=3,8
            nlinecol=nlinecol+1
...
c
            ...
            spoke_min= 10000.
            spoke_max=-10000.
            do n=-ndegrees+npsi_degrees,ndegrees+npsi_degrees
c
               theta=theta_offset+(delta_theta*float(n)) ! Keep away from 90
               thetb=theta+semidelta
               thetc=theta-semidelta
c
               call fb(theta,sx,sy,sr,qxa,qya,radius)
c
...
800            format(8f8.3)
c
               call fb(thetb,sx,sy,sr,qxb,qyb,radius)
               call fb(thetc,sx,sy,sr,qxc,qyc,radius)
c
...
108   format(6('   999.00'),/)
109   format('Displacement angle, nside (+/-1)?')
110   format('Radius_P=',f6.1,', Angle_P=',f6.1,' degrees'
     *      ,', Displacment angle=',f6.1,' degrees'
     *      ,', Px=',f6.1,', Py=',f6.1)
c
      end
c
c
c
      subroutine fb(theta,sx,sy,sr,qx,qy,radius)
c
      implicit real *8 (a-h,o-z)
c
      ca=1./cos(theta)**2
...
c
      if(cos(theta).gt.0.0
     *  )then
             qx=qx1   !  Return most positive root
      endif
      qy=qx*tan(theta)
c
      return
