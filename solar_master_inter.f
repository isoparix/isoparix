...
c
      real(8),allocatable,dimension(:) :: daily_total
c
      logical shad
c
      character(23)filename
      character(40),allocatable,dimension(:) :: label_txt
c                                                (0:39,0:5)
      call xxstatmap
c
      pi=355./113.
...
c
      ...
      daily_total=0.0
c
      allocate(label_txt(0:labchars_y))
      allocate(picdata(ixdim,iydim))
      allocate(daily_total(ixdim))
      daily_total=0.0
c
      ixelev=366
      iyelev=180
...
c
      ...
      do k=1,12
         ix=a*float(k)
         picdata(1+ix+ixsol,iysol:iydim)=char(1)
      enddo
c
...
c
     ...
     *           ,panel_insolation,panel_percent
      close(12)
...
c
      call label_maker(1,8,13,ixdim,iydim,picdata,label_chars
     *               ,labchars_x-1,labchars_y-1)
