...
c
      ...
      nkey=0		! C major
      kont=kont_def
      note_vel_def=96 	! Note velocity (S/s=5)
      note_vol_def=96 	! Note volume (U/u=5)
c
...
c
      track_data(mt(1)  ,1)=char(z'00')! Delta-time zero
      track_data(mt(1)+1,1)=char(z'F0')! SysEx
      track_data(mt(1)+2,1)=char(z'05')! Five bytes after this
      track_data(mt(1)+3,1)=char(z'7E')! Non-realtime
      track_data(mt(1)+4,1)=char(z'7F')! SysEx channel "disregard channel"
      track_data(mt(1)+5,1)=char(z'09')! GM system
      track_data(mt(1)+6,1)=char(z'01')! GM enable - 00 OFF, 01 ON, 02 ??
      track_data(mt(1)+7,1)=char(z'F7')! End of SysEx
      mt(1)=mt(1)+8
c
      track_data(mt(1)  ,1)=char(z'00')! Delta-time zero
      track_data(mt(1)+1,1)=char(z'FF')! Meta-event
...
c      Expression track
c
      track_data(mt(idtvol)  ,idtvol)=char(z'00')! Delta-time zero
      track_data(mt(idtvol)+1,idtvol)=char(z'FF')! Meta-event
      track_data(mt(idtvol)+2,idtvol)=char(z'03')! 0x03 Sequence/Track name
...
c
      if(text(i).eq.'X'
     *  )then
             call easy_midi_numerics(i+1,i+5)
...
c
     ...
     *          text(i+1).eq.'s'
     *         )then  
                    call easy_midi_numerics(i+2,i+3)
...
c
     ...
     *          text(i+1).eq.'e'
     *         )then
                    call easy_midi_numerics(i+2,i+6)
...
c
                    ...
                    mrest_time=0
c
                     npatch_chann=(16*12)+idt-4
                    ncon_on_chann=(16*11)+idt-4
                    note_on_chann=(16*9) +idt-4
c
...
c            delta_time=0
c            call make_midi_time(idt)  ! Writes the (zero) dt, advances mt(idt)
      endif
c
      get_next_note=.true.
c
...
c
     ...
     *         )then
c                   delta_time=min_length
                      lta_time=min_length
                      nm=note_stack(note_min_len)
                else
c                   delta_time=min_on
                      lta_time=min_on
                      nm=note_stack(note_min_on)
             endif
c
c            if(nm.eq.mrest
c    *         )then
c                   mrest_time=mrest_time+lta_time
c               else
                    delta_time=mrest_time+lta_time
                    mrest_time=0
c            endif
c       
             if(check
     ...
     *                           ,len_stack(mx),mx=nstack,1,-1),idt
...
c
     ...
     *                          )then
                                     write(12,1352)track_time(idt),idt
     *                                 ,ichar(track_data(mt(idt),idt))
     *                                 ,mt(idt)
                                     write(12,1353)idt
     *                                 ,ichar(track_data(mt(idt)+1,idt))
     *                                 ,mt(idt)+1
                              endif
c
                              mt(idt)=mt(idt)+2
