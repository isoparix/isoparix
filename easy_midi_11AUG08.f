...
c
             if(staccato
     *         )then
                    note_off_time=.1*float(3*delta_time)  ! 8 sounds best
                    note_on_time=delta_time-note_off_time
                    track_data(mt(idt)-1,idt)=char(note_on_time)
c
c      After this time reduce the volume to compensate
c
                    track_data(mt(idt)  ,idt)=char(ncon_on_chann)! Controller
                    track_data(mt(idt)+1,idt)=char(7)            ! Volume
                    track_data(mt(idt)+2,idt)=char(note_vol/3)   ! Value
                    track_data(mt(idt)+3,idt)=char(note_off_time)
                    track_data(mt(idt)+4,idt)=char(ncon_on_chann)! Controller
                    track_data(mt(idt)+5,idt)=char(7)            ! Volume
                    track_data(mt(idt)+6,idt)=char(note_vol)     ! Value
                    track_data(mt(idt)+7,idt)=char(0)
                    track_data(mt(idt)+8,idt)=char(note_on_chann)! Note ON
                    mt(idt)=mt(idt)+9
                else
                    track_data(mt(idt)-1,idt)=char(delta_time)
                    write(4,116)delta_time,mt(idt)-1
...
c
     ...
     *                                         =char(note_stack(istack))
                              ...
                              track_data(mt(idt)+2,idt)=char(0)	! Delta time 0
                 write(2,1221)track_data(mt(idt)  ,idt),mt(idt)
                 write(2,1221)track_data(mt(idt)+1,idt),mt(idt)+1
                 write(2,1222)track_data(mt(idt)+2,idt),mt(idt)+2
                              mt(idt)=mt(idt)+3
