...
c
     ...
     *                  )then
                             if(agrid(ix,iy).gt.10000..or.
     *                          bgrid(ix,iy).gt.10000.
     *                         )then
     ...
     *                                         , grid(ix-2:ix+2,iy)
                             ...
                             xstart(nrle+1)=bgrid(ix,iy)
                             ystart(nrle)  =real(iy)-0.5D+00
                             yend  (nrle)  =real(iy)-0.5D+00
                             ystart(nrle+1)=real(iy)+0.5D+00
                             yend  (nrle+1)=real(iy)+0.5D+00
                             found_depth=.true.
                             exit
                      endif
                   enddo
...
c
     ...
     *                      ,grid(idx,iy),agrid(idx,iy),bgrid(idx,iy)
     *                      ,grid(itx,iy),agrid(itx,iy),bgrid(itx,iy)
                                 if(itx.eq.idx
...
c
                          ...
                          if(xend(nrle  ).gt.10000..or.
     *                       xend(nrle+1).gt.10000.
     *                      )then
                                 write(0,108)line_start(iy),idx,itx,iy
     *                                         ,agrid(itx-2:itx+2,iy)
     *                                         ,bgrid(itx-2:itx+2,iy)
     *                                         , grid(itx-2:itx+2,iy)
                              stop
                          endif
                          nrle=nrle+2  !  Start a new line segment pair
                   endif   !  found_depth
            endif
c
         enddo
      ...
      enddo
      nrle=nrle-1    !  MARK THIS for no 'last-line' gaps....
c
...
101   format('GRIDMAP: idx,itx,iy,grid,agrid,bgrid:',3i6,2(i6,2f15.7))
